...
describe('createView', function() {
  ...
  it('creates view with subviews', function() {
    ...
    assert.deepEqual(Object.keys(view.spy.firstCall.args[0]), ['subView']);
  ...
  it('creates view with nested subviews', function() {
    ...
    assert.equal(v, 'view');
    sinon.assert.calledOnce(view.spy);
    assert.deepEqual(Object.keys(view.spy.firstCall.args[0]), ['subSubView', 'subView']);
    sinon.assert.calledOnce(subView.spy);
    assert.deepEqual(Object.keys(subView.spy.firstCall.args[0]), ['subProp', 'subSubView']);
  ...
  it('creates view with multiple subviews at a single step', function() {
    ...
    assert.equal(v, 'view');
    sinon.assert.calledOnce(view.spy);
    assert.deepEqual(Object.keys(view.spy.firstCall.args[0]), ['subView', 'sub2View']);
    sinon.assert.calledOnce(subView.spy);
