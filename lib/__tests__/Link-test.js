...
describe('Link', function() {
  ...
  describe('markup generation', function() {
    ...
    describe('generating href by referring to a relative route name', function() {
      ...
      var matchUsers = {
        route: users,
        trace: makeMatchTrace(routes, 'users')
      };
      ...
      var matchUser = {
        ...
        trace: makeMatchTrace(routes, 'users/user')
      };
    ...
    it('throws meaningful error if it cannot resolve route reference', function() {
      assert.throws(
        () => renderLink(<Link to="/x">link</Link>),
        Error,
  ...
  describe('onClick behaviour', function() {
    ...
    it('navigates using routing', function() {
      ...
      sinon.assert.calledWith(routing.navigate, '/users/me');
    });
